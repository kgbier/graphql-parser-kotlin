...
internal class GraphQl {
    ...
    val requiredTokenSeparator = oneOrMore(
        oneOf(
            listOf(
                comment,
                lineTerminator,
    ...
    val stringCharacter = oneOf(listOf(
        zip(
            notOneOf(listOf(literal("\""), literal("\\"), lineTerminator)),
            sourceChar
        ).map { (_, c) -> c },
